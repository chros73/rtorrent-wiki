...
If you know host name/IP and port of a DHT node, you can enter it manually with the command `dht.add_node=host:port`. This command only adds the node to the routing table if there is space for it.
